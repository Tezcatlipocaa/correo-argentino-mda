...
const asistenciaPage = await read(
  "src/pages/supervision/asistencia/index.astro",
);
assert.match(
  asistenciaPage,
  /import\s+AsistenciaContent\s+from\s+["']@components\/supervision\/asistencia\/AsistenciaContent\.astro["']/,
);
assert.match(
  asistenciaPage,
  /import\s+AsistenciaSkeleton\s+from\s+["']@components\/supervision\/asistencia\/AsistenciaSkeleton\.astro["']/,
);
assert.match(asistenciaPage, /<AsistenciaContent\s+server:defer\s*>/);
assert.match(
  asistenciaPage,
  /<AsistenciaSkeleton\s+slot=["']fallback["']\s*\/>/,
);

// 4. Check Estadisticas Page Defer
const estadisticasPage = await read(
  "src/pages/supervision/asistencia/estadisticas.astro",
);
assert.match(
  estadisticasPage,
  /import\s+EstadisticasContent\s+from\s+["']@components\/supervision\/asistencia\/EstadisticasContent\.astro["']/,
);
assert.match(
  estadisticasPage,
  /import\s+EstadisticasSkeleton\s+from\s+["']@components\/supervision\/asistencia\/EstadisticasSkeleton\.astro["']/,
);
assert.match(estadisticasPage, /<EstadisticasContent\s+server:defer\s*>/);
assert.match(
  estadisticasPage,
  /<EstadisticasSkeleton\s+slot=["']fallback["']\s*\/>/,
);
