...
export function getStatusStyles(type: string | undefined): {
  ...
  switch (type) {
    case OperatorStatus.HomeOffice:
      return {
        badge:
          "bg-secondary/10 text-secondary border border-secondary/20 font-bold px-2.5 py-1 rounded-full text-xxs tracking-wide uppercase whitespace-nowrap",
    ...
    case OperatorStatus.PresencialMonteGrande:
      return {
        badge:
          "bg-primary/10 text-amber-700 dark:text-amber-400 border border-primary/20 font-bold px-2.5 py-1 rounded-full text-xxs tracking-wide uppercase shadow-sm whitespace-nowrap",
    ...
    case OperatorStatus.PresencialParquePatricios:
      return {
        badge:
          "bg-purple-500/10 text-purple-700 dark:text-purple-400 border border-purple-500/20 font-bold px-2.5 py-1 rounded-full text-xxs tracking-wide uppercase shadow-sm whitespace-nowrap",
        icon: STATUS_ICONS.briefcase,
        color: "#a855f7",
        bgClass:
    ...
    case OperatorStatus.Licencia:
      return {
        badge:
          "bg-error/10 text-error border border-error/20 font-bold px-2.5 py-1 rounded-full text-xxs tracking-wide uppercase whitespace-nowrap",
    ...
    case OperatorStatus.Vacaciones:
      return {
        badge:
          "bg-success/10 text-success border border-success/20 font-bold px-2.5 py-1 rounded-full text-xxs tracking-wide uppercase whitespace-nowrap",
    ...
    default:
      return {
        badge:
          "bg-base-200 text-base-content/75 px-2.5 py-1 rounded-full text-xxs font-bold tracking-wide uppercase border border-base-300/50 whitespace-nowrap",
...
export function getDetailStatusClass(status: string): string {
  ...
  if (status === OperatorStatus.PresencialMonteGrande)
    return "text-amber-700 dark:text-amber-400";
  if (status === OperatorStatus.PresencialParquePatricios)
