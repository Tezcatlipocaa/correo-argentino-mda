...
export function updateNavigationButtons(): void {
  ...
  if (nextBtn) {
    const hasNext =
      currentIndex !== -1 && currentIndex < state.uniqueMonths.length - 1;
...
export function updatePasivaActiveMonthBadge(): void {
  ...
  const pasivaSelect = document.getElementById(
    "pasiva-month-selector",
...
export function updateGroupsActiveMonthBadge(): void {
  ...
  const groupsSelect = document.getElementById(
    "groups-month-selector",
  ) as HTMLSelectElement | null;
  ...
  const html = state.uniqueMonths
    .map((ymStr) => {
      ...
      const label = isCurrent
        ? `${MESES_ES[month]} ${year} (mes actual)`
        : `${MESES_ES[month]} ${year}`;
...
export function changeMonth(offset: number): void {
  ...
  if (newIndex >= state.uniqueMonths.length)
    newIndex = state.uniqueMonths.length - 1;
...
export function renderDaily(): void {
  ...
  const filteredOps = state.cronoData.filter((op) => {
    ...
    const matchesSearch =
      !state.searchQuery ||
      op.nombre.toLowerCase().includes(state.searchQuery.toLowerCase());
    if (!matchesSearch) return false;

    // 2. Location Filter
    if (
      state.activeLocationFilter !== "all" &&
      (op.location || "Monte Grande") !== state.activeLocationFilter
    ) {
      return false;
    }
  ...
  if (sortedOps.length === 0) {
    ...
    sortedOps.forEach((op) => {
      ...
      let liveStatus = isCurrentlyWorking(
        dailyHorario,
        customBreakInicio,
        customBreakFin,
        isWeekend,
      ...
      if (liveStatus.status === "online")
        liveIndicatorClass = "bg-success shadow-glow-success";
      else if (liveStatus.status === "break")
        liveIndicatorClass = "bg-warning shadow-glow-warning";

      let ringClass =
        "bg-base-200/50 text-base-content/60 ring-base-200 border border-base-300";
      let glowColorClass = "bg-primary";
      if (status === OperatorStatus.HomeOffice) {
        ringClass =
          "bg-secondary/10 text-secondary ring-secondary/30 border border-secondary/20 shadow-sm";
        glowColorClass = "bg-secondary";
      } else if (status === OperatorStatus.PresencialMonteGrande) {
        ringClass =
          "bg-primary/10 text-amber-700 dark:text-amber-400 ring-primary/30 border border-primary/20 shadow-sm";
        glowColorClass = "bg-amber-500";
      } else if (status === OperatorStatus.PresencialParquePatricios) {
        ringClass =
          "bg-purple-500/10 text-purple-700 dark:text-purple-400 ring-purple-500/30 border border-purple-500/20 shadow-sm";
      ...
      const isYesterdayAbsent =
        yesterdayStatus === OperatorStatus.Licencia ||
        yesterdayStatus === OperatorStatus.Vacaciones ||
        yesterdayStatus === OperatorStatus.Franco;

      if (yesterdayStatus && !isYesterdayAbsent) {
        const yesterdayHorario =
          (op.horarios_dias && op.horarios_dias[prevDateStr]) || op.horario;
        ...
        if (yTimes.length === 2) {
          ...
          if (yStartPct > yEndPctVal) {
            ...
            if (yesterdayStatus === OperatorStatus.HomeOffice)
              yesterdayWorkBarBg = "bg-secondary text-secondary-content";
            else if (
              yesterdayStatus === OperatorStatus.PresencialParquePatricios
            )
              yesterdayWorkBarBg = "bg-purple-500 text-white";
            ...
            const yIsWeekend =
              yDateObj.getDay() === 0 || yDateObj.getDay() === 6;
            if (!yIsWeekend) {
      ...
      if (showAsActiveInGantt) {
        ...
        if (times.length === 2) {
          ...
          if (status === OperatorStatus.HomeOffice)
            todayWorkBarBg = "bg-secondary text-secondary-content";
          else if (status === OperatorStatus.PresencialParquePatricios)
            todayWorkBarBg = "bg-purple-500 text-white";
          ...
          if (breakStartHourStr && breakEndHourStr) {
            ...
            if (bStartPct <= bEndPct) {
              breakBars.push(
                `<div class="gantt-bar-break relative z-20" style="left: ${bStartPct}%; width: ${bEndPct - bStartPct}%;" title="Descanso: ${breakStartHourStr} - ${breakEndHourStr}">${tooltipHtml}</div>`,
              );
            } else {
              breakBars.push(
                `<div class="gantt-bar-break relative z-20" style="left: ${bStartPct}%; width: ${100 - bStartPct}%;" title="Descanso: ${breakStartHourStr} - ${breakEndHourStr}">${tooltipHtml}</div>`,
              );
              breakBars.push(
                `<div class="gantt-bar-break relative z-20" style="left: 0%; width: ${bEndPct}%;" title="Descanso: ${breakStartHourStr} - ${breakEndHourStr}">${tooltipHtml}</div>`,
              );
            }
          }
        }
      ...
      if (isAbsent || isFranco) {
        ...
        ganttContentHtml = `
          ...
          ${
            !isOvernight &&
            tEndPct < 100 &&
            (!dayOvertime || tEndPct >= getPct(dayOvertime.startTime))
              ? `
      ...
      const timelineBg = isHoliday
        ? "bg-orange-100/10 dark:bg-orange-950/10"
        : "";
...
export function renderHourly(dateStr: string): void {
  ...
  if (sortedOps.length === 0) {
    ...
    sortedOps.forEach((op) => {
      ...
      let rowClass =
        "group hover:bg-base-200/40 transition-colors border-b border-base-200/50";
      if (isHoliday) {
        rowClass +=
          " bg-orange-100/30 dark:bg-orange-950/30 pointer-events-none";
      }
      let tdClass =
        "sticky left-0 bg-base-100 z-30 w-[200px] min-w-[200px] font-bold py-3 px-6 text-xs border-r border-base-200/70 group-hover:bg-base-200 transition-colors";
      ...
      let opNameBtnClass =
        "hover:text-secondary hover:underline underline-offset-2 transition-colors text-left truncate flex-1 font-bold text-xs";
      if (isHoliday) {
      ...
      hours.forEach((hour, i) => {
        const working =
          hS !== -1 &&
          hE !== -1 &&
          (hS <= hE ? i >= hS && i < hE : i >= hS || i < hE);
        const onBreak =
          working &&
          bhS !== -1 &&
          bhE !== -1 &&
          (bhS <= bhE ? i >= bhS && i < bhE : i >= bhS || i < bhE);
        if (onBreak) {
...
export function renderMonthly(): void {
  ...
  const monthLongFormatter = new Intl.DateTimeFormat("es-AR", {
    month: "long",
  });
  ...
  const parsedDates = dates.map((date) => {
    ...
    const isCritical =
      teamSize > 0
        ? coverage.total / teamSize < state.minCoveragePercent / 100
        : false;
    ...
    if (isToday)
      ...
      thClass = thClass.replace(
        "bg-base-100",
        "bg-orange-200 dark:bg-orange-800 text-orange-800 dark:text-orange-100 font-bold line-through",
      );
    else if (isWeekend)
      thClass = thClass.replace(
        "bg-base-100",
  ...
  parsedDates.forEach((pd) => {
    ...
    const tooltipText = pd.feriadoName
      ? `Feriado: ${pd.feriadoName}`
      : `Ver detalle del día ${pd.dateNum} de ${pd.monthName}`;
    theadHtml += `
  ...
  if (sortedOps.length === 0) {
    ...
    sortedOps.forEach((op, opIdx) => {
      ...
      const isCurrentUser = !!(
        currentUsername &&
        (opUsername === currentUsername || opNameLower === currentUsername)
      );
      ...
      parsedDates.forEach((pd) => {
        ...
        const { status, horario: dailyHorario } =
          resolveOperatorStatusAndHorario(op, date, pd.day, pd.activeGroup);
        const styles = getStatusStyles(status);
        ...
        const isLicenseOverlap =
          (status === OperatorStatus.Licencia ||
            status === OperatorStatus.Vacaciones) &&
          coveragePerDay[date].licenses > state.maxLicenseOverlapLimit;
        ...
        const hasComment =
          !hideComments && !!(op.comentarios && op.comentarios[date]);
        const dayOvertime = (op.weekendOvertimes || []).find(
          (s) => s.date === date,
        );
        ...
        const isRotationCell = !!(
          isSaturday &&
          op.saturdayGroup &&
          op.saturdayGroup === pd.activeGroup &&
  ...
  const todayHeader = document.querySelector(
    '[data-today-header="true"]',
  ) as HTMLElement | null;
  if (todayHeader) {
    const scrollContainer =
      document.getElementById("monthly-table")?.parentElement;
    if (scrollContainer) {
      requestAnimationFrame(() => {
        const stickyCol = document.querySelector(
          "#monthly-thead th.sticky",
        ) as HTMLElement | null;
        ...
        const targetScrollLeft = Math.max(
          0,
          todayLeft - stickyWidth - visibleDaysWidth / 2 + todayWidth / 2,
        );
