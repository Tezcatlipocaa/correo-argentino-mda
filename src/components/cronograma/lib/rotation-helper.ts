...
export let activeRotationConfig: {
  startDate: string;
  startGroup: string;
...
export function isHourCoveredBySchedule(
  scheduleStr: string,
  hourStart: number,
): boolean {
...
export function getActiveGroupForDate(
  dateStr: string,
...
export function getNextSaturdayForGroup(targetGroup: string): string | null {
  ...
  const activeMonthPrefix = dateInput?.value
...
export function renderRotationTimeline(dateStr: string): void {
  ...
  if (groupDisplay) {
    groupDisplay.textContent = activeGroup
      ? `Grupo ${activeGroup}`
      : "Sin grupo";
    if (activeGroup) {
  ...
  const groupOps = state.cronoData.filter(
    (op) => op.saturdayGroup === activeGroup,
  );
  ...
  const badgeClass =
    groupBadgeClasses[activeGroup] || "bg-emerald-500 text-white";

  tableBody.innerHTML = groupOps
    .map((op) => {
...
export function setupRotationEventListeners(): void {
  const rotationTimelineWrapper = document.getElementById(
    "rotation-timeline-date-wrapper",
  );
