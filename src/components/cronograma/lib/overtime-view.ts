...
export function renderOvertimeView(): void {
  const referenteSelect = document.getElementById(
    "overtime-referente-select",
  ) as HTMLSelectElement | null;
  const shiftAgentSelect = document.getElementById(
    "overtime-shift-agent",
  ) as HTMLSelectElement | null;
  ...
  if (referenteSelect) {
    ...
    if (textEl) {
      textEl.textContent = referenteSelect.value
...
export async function refreshOvertimeForWeekend(
  weekendDate: string,
): Promise<void> {
  ...
  let existingConfig = state.overtimeConfigs.find(
    (c) => c.weekendStartDate === weekendDate,
  );
  if (!existingConfig) {
    try {
      const res = await fetch(
        `/api/cronograma/overtime/config?weekendStartDate=${weekendDate}`,
      );
      if (res.ok) {
  ...
  if (referenteSelect) {
    ...
    if (textEl) {
      textEl.textContent =
        existingConfig && existingConfig.referente
          ? existingConfig.referente
          : "Sin asignar";
    }
  }
  ...
  try {
    const res = await fetch(
      `/api/cronograma/overtime/shifts?weekendStartDate=${weekendDate}`,
    );
    ...
    state.cronoData.forEach((op) => {
      const otherWeekends = (op.weekendOvertimes || []).filter(
        (s) => s.weekendStartDate !== weekendDate,
      );
...
export function renderOvertimeTimeline(
  weekendDate: string,
  shifts: WeekendOvertimeShift[],
): void {
  ...
  const activeOps = state.cronoData
    .filter(
      (op) => op.id !== undefined && shifts.some((s) => s.agentId === op.id),
    )
    .sort((a, b) => {
  ...
  bodyContainer.innerHTML = activeOps
    .map((op) => {
...
export function loadShiftIntoForm(
  ...
  if (daySelect)
    daySelect.value = shift.date === weekendDate ? "saturday" : "sunday";
...
export function renderOvertimeShiftsList(
  weekendDate: string,
  shifts: WeekendOvertimeShift[],
): void {
...
export function setupOvertimeEventListeners(): void {
  ...
  document
    .getElementById("save-overtime-referente-btn")
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        const idx = state.overtimeConfigs.findIndex(
          (c) => c.weekendStartDate === saved.weekendStartDate,
        );
  ...
  if (overtimeShiftForm) {
    overtimeShiftForm.addEventListener("submit", async (e) => {
      ...
      const agentIdVal = (
        document.getElementById("overtime-shift-agent") as HTMLSelectElement
      ).value;
      const dayVal = (
        document.getElementById("overtime-shift-day") as HTMLSelectElement
      ).value;
      const startTime = (
        document.getElementById("overtime-shift-start") as HTMLInputElement
      ).value;
      const endTime = (
        document.getElementById("overtime-shift-end") as HTMLInputElement
      ).value;
      const editId = (
        document.getElementById("overtime-shift-edit-id") as HTMLInputElement
      ).value;
      ...
      const sundayDateObj = new Date(
        state.overtimeSelectedWeekend + "T12:00:00",
      );
      sundayDateObj.setDate(sundayDateObj.getDate() + 1);
      const sundayDate = `${sundayDateObj.getFullYear()}-${String(sundayDateObj.getMonth() + 1).padStart(2, "0")}-${String(sundayDateObj.getDate()).padStart(2, "0")}`;
      const shiftDate =
        dayVal === "saturday" ? state.overtimeSelectedWeekend : sundayDate;

      const submitBtn = document.getElementById(
        "overtime-shift-submit-btn",
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      const shiftId = (
        document.getElementById("overtime-shift-delete-btn") as HTMLElement
      ).dataset.shiftId;
      if (!shiftId || !state.overtimeSelectedWeekend) return;
      const confirmed = await showConfirm(
        "¿Eliminar este turno de hora extra?",
      );
      if (!confirmed) return;
      try {
        const res = await fetch(
